...
- 拍照上传面部照片，完整分析管线：
  ...
  2. **BiSeNet Face Parsing（ONNX）** — 分割面部皮肤与头发，提取可见发际线
  3. **ROI 提取** — 分割蒙版限定前额等 8 个皮肤区域，排除头发、眼眉和唇体
  4. **Feature Extraction** — 纯 numpy/scipy 数值计算，从每区提取颜色 / 纹理 / 毛孔 / 斑点 / 光泽特征，Sigmoid 映射为 0-100 评分
  5. **RAG 知识增强** — 从本地 JSON 知识库按肤质和关注点检索参考知识，不额外调用 Embedding
  6. **Gemini 自然语言生成** — 只基于 Feature JSON 生成总结和建议，**不看图不评分**
  7. **热力图** — matplotlib 高斯扩散渲染面部热力图
- 分析历史自动保存

### 三庭五眼教程推荐
- 镜前检测复用同一次 Face Parsing 结果，以发际线—眉间距离计算上庭
- 刘海遮挡、发际线裁切或模型不可用时不强行判断上庭，并提示重新拍摄
- 同步计算五眼和面部长宽比例
...
# 下载并校验发际线分割 ONNX 模型（约 51MB）
python backend/download_face_parsing_model.py
...
启动后可访问 `http://127.0.0.1:5000/api/health/face-parsing`。当
`face_parsing.ready` 为 `true` 时，肤质额头 ROI 和三庭上庭均在使用 ONNX
分割；为 `false` 时会安全回退到 MediaPipe 几何蒙版。

Render 的 Docker 构建会自动下载并校验该模型，不需要手工把 51MB 模型提交到 Git。
...
│   ├── face_parsing.py           # ONNX 皮肤/头发分割 + 可见发际线
│   ├── face_ratio_analyzer.py    # 三庭五眼比例分析
...
│   │   └── face_parsing_resnet18.onnx # 本地下载；Git 忽略，Docker 自动获取
│   └── uploads/
│       ├── products/
│       ├── diary/
│       └── skin/
...
| 人脸分割 | BiSeNet ResNet18 + ONNX Runtime | 19 类面部分割，提取皮肤 / 头发 / 发际线 |
